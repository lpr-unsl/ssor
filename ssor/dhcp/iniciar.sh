...
docker run --detach --hostname clienteLan2 -it --name clienteLan2 --cap-add NET_ADMIN cliente-cli:1.5 bash
docker run --detach --hostname potrero -it --name potrero --cap-add NET_ADMIN --privileged servidor:1.5 bash
#docker run --detach --hostname laflorida -it --name laflorida --cap-add NET_ADMIN --privileged servidor:1.5 bash
docker run --detach --hostname merlo -it --name merlo --cap-add NET_ADMIN cliente-cli:1.5 bash
docker run --detach --hostname clienteLan1 -it --name clienteLan1 --cap-add NET_ADMIN cliente-cli:1.5 bash
docker run --detach --hostname sanfelipe -it --name sanfelipe --cap-add NET_ADMIN --privileged servidor:1.5 bash
