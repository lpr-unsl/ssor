...
docker create --network=bridge --hostname clienteLan1 --name clienteLan1 -it --cap-add NET_ADMIN $hub-cliente-cli:$version
docker create --network=bridge --hostname sanfelipe --name sanfelipe -it --cap-add NET_ADMIN --privileged $hub-servidor-$svc:$version
