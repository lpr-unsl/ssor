...
docker run --detach --hostname potrero -it --name potrero --cap-add NET_ADMIN router:$version bash
...
docker run --detach --hostname carrizal -it --name carrizal --cap-add NET_ADMIN router:$version bash
docker run --detach --hostname laslenias -it --name laslenias --cap-add NET_ADMIN --env="DISPLAY" --volume="/tmp/.X11-unix:/tmp/.X11-unix:rw" cliente:$version bash
