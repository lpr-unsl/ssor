...
docker run --detach --hostname potrero -it --name potrero --cap-add NET_ADMIN router:1.6 bash
...
docker run --detach --hostname carrizal -it --name carrizal --cap-add NET_ADMIN router:1.6 bash
docker run --detach --hostname laslenias -it --name laslenias --cap-add NET_ADMIN cliente:1.6 bash
