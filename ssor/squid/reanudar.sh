...
docker run --detach --hostname clienteLan2 -it --name clienteLan2 --cap-add NET_ADMIN squid-clienteLan2 bash
docker run --detach --hostname merlo -it --name merlo --cap-add NET_ADMIN squid-merlo bash
docker run --detach --hostname clienteLan1 -it --name clienteLan1 --cap-add NET_ADMIN --env="DISPLAY" --volume="/tmp/.X11-unix:/tmp/.X11-unix:rw" squid-clienteLan1 bash
...
docker run --detach --hostname desaguadero -it --name desaguadero --cap-add NET_ADMIN --privileged squid-desaguadero bash
docker run --detach --hostname nogoli -it --name nogoli --cap-add NET_ADMIN --privileged squid-nogoli bash
