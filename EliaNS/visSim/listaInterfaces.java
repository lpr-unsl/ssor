...
public class listaInterfaces extends Vector
{
	...
	public void borra(String nombre, String ip, String mascara, String dirEnlace, String conecta)
	{
		...
		for (int i=0; i<size() && !encontrado; i++)
			if (getNombre(i).compareTo(nombre)==0 && getIP(i).compareTo(ip)==0 &&
			    getMascara(i).compareTo(mascara)==0 && getDirEnlace(i).compareTo(dirEnlace)==0 &&
			    getconecta(i).compareTo(conecta)==0)
				{
					removeElementAt(i);
					encontrado = true;
				}
		...
		if (!encontrado)
			System.err.println("Error inesperado. Interfaz " + nombre + " no encontrada.");
	}
	...
	public Vector borraConectado(String nombre)
	{
		...
		for (int i=0; i<size(); i++)
			if (getconecta(i).compareTo(nombre)==0)
			{
				dev.add(getIP(i));
				removeElementAt(i);
			}
	...
	{
		for (int i=0; i<size(); i++)
			if (getconecta(i).compareTo(nombreViejo)==0)
				setconecta(i, nombreNuevo);
	}
	...
	public String dameMACNoExistente(Vector listaMAC)
	{
		...
		while (asignada)
		{
			...
			while (MACnum.length()!=12)
				MACnum = "0"+MACnum;
			
			// Nos construimos una MAC a partir del numero de interfaces creadas en toda la topologia
			for (i=0; i<5; i++)
				MACtxt = MACtxt + MACnum.substring(i*2,(i*2)+2) + ":";
			MACtxt = MACtxt + MACnum.substring(10,12);
			
			// Ahora buscamos la mac nueva entre las existentes en toda la topologia
			encontrada = false;
			for (i=0; i<listaMAC.size() && !encontrada; i++)
				if (((String)listaMAC.elementAt(i)).compareTo(MACtxt)==0)
					encontrada = true;
			
			// Ahora la buscamos entre las de esta maquina
			if (!encontrada)
				for (i=0; i<tam(); i++)
					if (getDirEnlace(i).compareTo(MACtxt)==0)
						encontrada=true;

			// Si despues de todo la encontramos probamos con otra			
			if (encontrada)
				tamInicial++;
			// En caso contrario la devolvemos
			else
				asignada = false;
		}
	...
	public Vector dameNombresInterfaz(boolean copioDatoActual, String nombre)
	{
		...
		for (int i=0; i<tam(); i++)
			if (copioDatoActual || (!copioDatoActual && getNombre(i).compareTo(nombre)!=0))
				dev.add(getNombre(i));
	...
	public void modifica(String nombre, String ip, String mascara, String dirEnlace, String conecta, String nombreN, String ipN, String mascaraN, String dirEnlaceN, String conectaN)
	{
		...
		if (!encontrado)
			System.err.println("Error inesperado. Interfaz " + nombre + " no encontrada en listaInterfaces.modifica");
	}
	...
	/* Devuelve el numero de interfaces de la lista */
	public int tam()
	{
		return size();
	}
	...
	public boolean existeInterfaz(String nombre){
		for(int i = 0; i < size(); i++ )
			if(nombre.compareTo(this.getNombre(i)) == 0)
				return true;
		return false;
	}
}