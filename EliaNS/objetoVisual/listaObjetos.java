...
public class listaObjetos extends listaObjetosVisuales
{
	...
	public int buscaEquipo(String nombre)
	{
		...
		for (int i=0; i<tam; i++)
			if (getNombre(i).compareTo(nombre) == 0)
				return i;
	...
	public int buscaEquipo(String nombre, int pos)
	{
		...
		for (int i=0; i<tam; i++)
			if (i!=pos && getNombre(i).compareTo(nombre) == 0)
				return i;
	...
	public void cambiaCoord(int x, int y)
	{
		...
		for (i=0; i<tam; i++)
		{
			// Buscamos el mas cercano al origen
			
			// Distancia = Math.sqrt( (x2-x1)*(x2-x1) + (y2-y1)*(y2-y1));
			// Al comparar la distancia con respecto al 0,0 x1 e y1 valen 0
		...
		//Recorremos todo el vector de seleccionados para cambiar sus coordenadas
		for (i=0; i<tam; i++)
			if (i==indice)
				((objetoVisual)elementAt(i)).setCoord(x, y);
			else
				((objetoVisual)elementAt(i)).setCoord(getX(i)-xTras+x, getY(i)-yTras+y);
	}
	...
	public void cambiaNombres(listaObjetos listaOriginal)
	{
		...
		// Parche para que al pegar, si no hay nada antes, no se lie con los nombres
		if (listaOriginal.tam()==0) return;
		
		// En estos vectores almacenamos los cambios de los nombres
		// para luego poder cambiar todas las conexiones
		Vector nuevos, viejos, tmpConexion;
		objetoVisual eq;
		
		//Crear dos vectores para controlar el renombrado de las maquinas
		//en uno ira el nombre de la clase que se renombra y en el otro en la misma posicion
		//la cantidad de renombrados que se han hecho.
		// Esto se hace para mantener la numeracion en la lista de equipos que se van a pegar
		...
		for (i=0; i<tam; i++)
		{
			// Primero buscamos la cantidad de elementos renombrados e incrementamos en una unidad si ya existe
			ctd = -1;
			encontrado = false;
			...
			// Si no existe metemos en controlCambios el nuevo tipo de maquina de ese tipo
			if (ctd==-1)
			{
				ctd=0;
				t2 = new Vector();
			...
			// Cambiamos el nombre del equipo actual
		...
		// Recorremos nuevamente todo el vector para cambiar nombres en conexiones
		for (i=0; i<tam; i++)
		{
			tmpConexion = new Vector(getConexiones(i));
			
			for (j=0; j<tmpConexion.size(); j++)
				for (k=0; k<viejos.size(); k++)
					if (((String)tmpConexion.elementAt(j)).compareTo((String)viejos.elementAt(k))==0)
						tmpConexion.set(j, nuevos.elementAt(k));
	...
	public void centrarHorizontal(int altura, int anchura)
	{
		...
		// Para tener la distancia con respecto al limite inferior
		int dif = anchura/2 - (int)(new Point(getCM(altura, anchura))).getX();
		
		for (int i=0; i<tam; i++)
			if (getSeleccionado(i))
				setX(i, getX(i)+dif);
	}
	...
	public boolean compatiblesConexion()
	{
		...
		for (int i = ind1+1; i<tam && !encontrado; i++)
			if (getConecta(i))
			{
				encontrado = true;
				ind2 = i;
		...
		if (encontrado)
		{
			...
			// id2 hace referencia al identificador de obj. id1 es el primer equipo sobre el que se
			// hizo click para conectar
	...
	public Vector compruebaSimulacion()
	{
		...
		for (i=0; i<tam; i++)
		{
			...
			// Hacemos comprobaciones sobre pc y sobre router (alguno +?)
			if (eqtemp instanceof pcVisual || eqtemp instanceof routerVisual)
			{
				...
				// Por cada conexion hay que ver si esta su correspondiente interfaz
				for (j=0; j<conexiones.tam(); j++)
					if ((new Vector(interfaces.buscaConectados((String)conexiones.elementAt(j)))).size()==0)
						dev.add(getNombre(i) + " - No existen interfaces para la conexion con " + (String)conexiones.elementAt(j));
				
				if (rutas.tam()>0 && interfaces.tam()==0 && conexiones.tam()==0)
					dev.add(getNombre(i) + " - Existen entradas en la tabla de rutas que no corresponden a ninguna conexion ni interfaz.");
			}
			// Anyadimos mensajes para aquellos tipos de equipos para los que no se ha implementado la simulacion
			else if (eqtemp instanceof puenteVisual || eqtemp instanceof modemVisual || eqtemp instanceof wanVisual || eqtemp instanceof anilloVisual)
				dev.add(getNombre(i) + " - No se ha implementado la simulacion para este tipo de equipo");
		}
	...
	{
		for (int i=0; i<listatemp.tam(); i++)
			insertaObjeto((objetoVisual)listatemp.elementAt(i));
	}
	...
	public Vector copiaSeleccionados()
	{
		...
		for (int i=0; i<tam; i++)
			if (getSeleccionado(i) && getNomIcono(i).compareTo(nomiconos.nomVacio)!=0)
				dev.addElement(getNombre(i));
	...
	public void creaConexion()
	{
		...
		for (i=0; i<tam && !conectados && sePuede; i++)
			if (getConecta(i))
				// Ahora buscamos el otro equipo con el que conecta este
				for (j=i+1; j<tam && !conectados && sePuede; j++)
					if (getConecta(j))
					{
						// Si primer objeto es un modem
						if (getID(i).compareTo("mo")==0)
						{
							if (getConexiones(i).tam()==2 || (getID(j).compareTo("mo")==0 && getConexiones(j).tam()==2))
								sePuede = false;
							else if (getConexiones(i).tam()==1)
							{
								nombreConexion = getConexiones(i).getNombre(0);

								// Ya tiene una conexion con un modem entonces no conectamos
								if (getID(buscaEquipo(nombreConexion)).compareTo("mo")==0 && getID(j).compareTo("mo")==0)
										sePuede = false;

								// Ya tiene una conexion con otra cosa distinta a pc entonces tampoco conectamos
								if (sePuede && getID(buscaEquipo(nombreConexion)).compareTo("mo")!=0 && getID(j).compareTo("mo")!=0)
										sePuede = false;
								
								if (sePuede && getConexiones(j).contains(nombreConexion))
									sePuede = false;
							}
						}
						...
						{ 
							if (getConexiones(j).tam()==2  || (getID(i).compareTo("mo")==0 && getConexiones(j).tam()==2))
								sePuede = false;
							else if (getConexiones(j).tam()==1)
							{
								nombreConexion = getConexiones(j).getNombre(0); 

								// Ya tiene una conexion con un modem entonces no conectamos
								if (getID(buscaEquipo(nombreConexion)).compareTo("mo")==0 && getID(i).compareTo("mo")==0)
										sePuede = false;

								// Ya tiene una conexion con otra cosa distinta a pc entonces tampoco conectamos
								if (sePuede && getID(buscaEquipo(nombreConexion)).compareTo("mo")!=0 && getID(i).compareTo("mo")!=0)
										sePuede = false;

								if (sePuede && getConexiones(i).contains(nombreConexion))
									sePuede = false;
							}
						}
						...
						if (sePuede)
						{
							...
							if (pos1!=-1)
								getObjeto(i).setInterfazConecta(pos1, getNombre(j));

							if (pos2!=-1 && !getCadenaSimulador(j).startsWith("Ethernet"))
								getObjeto(j).setInterfazConecta(pos2, getNombre(i));
						}
						
						// y ponemos conecta a false
						setConecta(i, false);
						setConecta(j, false);
	...
	public Vector creaLineas()
	{
		...
		for (i=0; i<tam; i++)
		{
			...
			// Hacemos un nuevo recorrido para buscar los equipos con los que conecta el actual
			for (j=i+1; j<tam; j++)
				for (k=0; k<conexiones.tam(); k++)
					// Creamos una nueva linea
					if ((conexiones.getNombre(k)).compareTo(getNombre(j)) == 0)
						// Insertamos en el vector lineas
						lineas.add(getutilLinea(i, j));
		}
	...
	public void creaSelecciones(utilLinea rectangulo)
	{
		...
		// Segun las coordenadas del rectangulo el area puede variar
		...
		// Ahora recorremos la lista para ver que objetos se encuentran en
		/*  (x1, y1)
		    +-------
		    |      |
		    -------+ (x2, y2)
		*/
		...
		// Seleccion de los objetos por las 4 esquinas y de manera completa
		for (int i=0; i<tam; i++)
		{
			if((getNombre(i).length()>=5 && getNombre(i).substring(0,5).compareTo("*tex*")!=0) || (getNombre(i).length()<5))
			{
				...
				// Objeto dentro del area de seleccion
				if (x>=x1 && y>=y1 && x<=x2 && y<=y2)
					setSeleccionado(i, true);
				else
				{
					x=x+getWidth(i);
	...
	public int devMaximo(String nomicono)
	{
		...
		/* Para eliminar la dependencia con nombres hay que ver el nombre de la clase maquina y contar */

		String cadena = (new nomiconos()).getNombre(nomicono);
		String nombreClase = "objetoVisual." + cadena.toLowerCase() + "Visual";
		
		// Contamos objetos de la misma clase que el recibido		
		ctd=0;
		for (i=0; i<tam; i++)
			if (elementAt(i).getClass().getName().compareTo(nombreClase)==0)
				ctd++;
		...
		// Formamos el nombre como cadena y ctd y buscamos equipos con ese mismo nombre
		// cuando no se encuentre ninguno entonces se devuelve ctd.
		while (!asignado)
		{
			encontrado = false;
			for (i=0; i<tam && !encontrado; i++)
				if (getNombre(i).compareTo(nombre)==0)
					encontrado = true;
	...
	public void eliminaNoExistentes(Vector nombres)
	{
		...
		for (int i=0; i<tam; i++)
			getConexiones(i).eliminaNoExistentes(nombres);
	}
	...
	public void eliminaSeleccionados()
	{
		...
		for (int i=0; i<size(); )
			if (getSeleccionado(i))
			{
				// Eliminamos todas las conexiones que van a/desde este equipo
				nombre = getNombre(i);
	...
	private Point getCM(int altura, int anchura)
	{
		...
		// Buscamos el equipo que esta mas alto y mas bajo a la vez
		for (int i=0; i<tam; i++)
			if (getSeleccionado(i))
			{
				// El mas a la izquierda
				if (getX(i) < izq.getX())
					izq.setLocation(getX(i), getY(i));
				...
				// El mas alto
				if (getY(i) < alto.getY())
					alto.setLocation(getX(i), getY(i));
	...
	public Vector getDirecsMac(listaInterfaces interfaces, int pos)
	{
		...
		for (int i=0; i<tam; i++)
			if (i!=pos)
			{
				...
				for (j=0; j<tempInter.tam(); j++)
					dev.add(tempInter.getDirEnlace(j));
			}

		// Anyadimos las direcciones de las interfaces del equipo actual
		for (j=0; j<interfaces.tam(); j++)
			dev.add(interfaces.getDirEnlace(j));
		...
		for (int i=0; i<interfaces.tam(); i++)
			if (interfaces.getconecta(i).compareTo("N/A")!=0)
			{
				...
				if (pos2!=-1)
				{
					...
					// Para que no se de una excepcion cuando la longitud sea 0
					if (h==0) h++;
	...
	public Vector getListaIPConecta(Vector nombresRedes, String noCopiar)
	{
		...
		for (i=0; i<tam; i++)
			if (getID(i).compareTo("pc")==0 || getID(i).compareTo("ro")==0)
			{
				...
				for (j=0; j<interfaces.tam(); j++)
					if (noCopiar.compareTo(interfaces.getIP(j))!=0 && nombresRedes.contains(interfaces.getconecta(j)))
						dev.add((interfaces.getIP(j) + " - " + interfaces.getconecta(j)));
			}
	...
	public Vector getNombres()
	{
		...
		for (int i=0; i<tam(); i++)
			if (getCadenaSimulador(i).length()>0)
				dev.add(getNombre(i));
	...
	public Vector getNombresOrdenadores()
	{
		...
		for (int i=0; i<tam(); i++)
			if (getID(i).compareTo("pc")==0)
			{
				...
				for (j=0; j<interfaces.tam(); j++)
					// Solo de interfaces conectadas a algun sitio
					if (interfaces.getconecta(j).compareTo("N/A")!=0)
						dev.add(getNombre(i)+ " (" + interfaces.getIP(j) + ")");
			}
	...
	public Vector getTipos(Vector conexiones)
	{
		...
		for (int i=0; i<conexiones.size(); i++)
			if (buscaEquipo((String)conexiones.elementAt(i))!=-1)
				dev.add(getID(buscaEquipo((String)conexiones.elementAt(i))));
	...
	public void mueveEquipos(int x, int y)
	{
		...
		for (int i=0; i<tam; i++)
			((objetoVisual)elementAt(i)).setCoord(getX(i)+x, getY(i)+y);
	}
	...
	public void mueveEquiposSeleccionados(int xRaton, int yRaton)
	{
		...
		for (int i=0; i<tam; i++)
			if (getSeleccionado(i))
				((objetoVisual)elementAt(i)).setCoord(getX(i)-incDistanx+xRaton, getY(i)-incDistany+yRaton);
	}
	...
	public void pega(listaObjetos original, Vector nombres)
	{
		...
		for (int i=0; i<nombres.size(); i++)
		{
			...
			if (pos!=-1)
				insertaObjeto((objetoVisual)original.getObjeto(pos).clone());
		}
	}
	...
	public boolean posibleSimular()
	{
		...
		for (int i=0; i<size() && !dev; i++)
			if (getID(i).compareTo("pc")==0)
				if (getConexiones(i).size()>0)
					dev = true;
